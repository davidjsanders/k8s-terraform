...
resource "azurerm_virtual_machine" "vm-jumpbox" {
  ...
  name                             = "${upper(format("VM-JUMP-%s-%s%s", var.target, var.environ, local.l-random))}"
  ...
  storage_os_disk {
    name              = "${format("OSD-JUMPBOX-%s-%s%s", var.target, var.environ, local.l-random)}"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "${format("%s-JUMPBOX-%s-%s%s", var.vm-name, var.target, var.environ, local.l-random)}"
