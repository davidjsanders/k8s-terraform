...
# Submodule:      nsg-allow-443-lb.tf
# Environments:   all
# Purpose:        Module to define the Azure network security group
#                 rule to allow port 443 to the load balancer.
#
...
resource "azurerm_network_security_rule" "k8s-nsgrule-allow-443-lb" {
  name                        = "${format("NSG-ALLOW-443-LB-%s-%s%s", var.target, var.environ, local.l-random)}"
  priority                    = 1020
  direction                   = "Inbound"
  access                      = "Allow"
  protocol                    = "Tcp"
